; ModuleID = '/home/acce2632/acce-fpga/.small_dam_base_disabled/solution_FLOOD_HLS_base/.autopilot/db/a.g.ld.5.gdce.bc'
source_filename = "llvm-link"
target datalayout = "e-m:e-i64:64-i128:128-i256:256-i512:512-i1024:1024-i2048:2048-i4096:4096-n8:16:32:64-S128-v16:16-v24:32-v32:32-v48:64-v96:128-v192:256-v256:256-v512:512-v1024:1024"
target triple = "fpga64-xilinx-none"

%struct.parameters = type { [90 x [90 x float]], float, i32, float, [9 x %struct.Cloud_t] }
...
entry:
  %malloccall.0 = call i8* @malloc(i64 32400)
  %p_copy.0 = bitcast i8* %malloccall.0 to [90 x [90 x float]]*
  ...
  call fastcc void @copy_in(%struct.parameters* nonnull %p, [90 x [90 x float]]* %p_copy.0, float* %p_copy.1, i32* %p_copy.2, float* %p_copy.3, [9 x i224]* %p_copy.4, %struct.results* nonnull %r, i416* nonnull align 512 %r_copy)
  call void @apatb_do_compute_hw([90 x [90 x float]]* %p_copy.0, float* %p_copy.1, i32* %p_copy.2, float* %p_copy.3, [9 x i224]* %p_copy.4, i416* %r_copy)
  call void @copy_back(%struct.parameters* %p, [90 x [90 x float]]* %p_copy.0, float* %p_copy.1, i32* %p_copy.2, float* %p_copy.3, [9 x i224]* %p_copy.4, %struct.results* %r, i416* %r_copy)
...
define internal fastcc void @copy_in(%struct.parameters* noalias readonly "unpacked"="0", [90 x [90 x float]]* noalias "unpacked"="1.0" %.0, float* noalias nocapture "unpacked"="1.1" %.1, i32* noalias nocapture "unpacked"="1.2" %.2, float* noalias nocapture "unpacked"="1.3" %.3, [9 x i224]* noalias "unpacked"="1.4" %.4, %struct.results* noalias readonly "unpacked"="2", i416* noalias align 512 "unpacked"="3") unnamed_addr #1 {
entry:
  call fastcc void @onebyonecpy_hls.p0struct.parameters([90 x [90 x float]]* %.0, float* %.1, i32* %.2, float* %.3, [9 x i224]* %.4, %struct.parameters* %0)
  call fastcc void @onebyonecpy_hls.p0struct.results.216(i416* align 512 %2, %struct.results* %1)
  ret void
}

; Function Attrs: argmemonly noinline norecurse willreturn
define internal fastcc void @onebyonecpy_hls.p0struct.parameters([90 x [90 x float]]* noalias "unpacked"="0.0" %dst.01, float* noalias nocapture "unpacked"="0.1" %dst.12, i32* noalias nocapture "unpacked"="0.2" %dst.23, float* noalias nocapture "unpacked"="0.3" %dst.34, [9 x i224]* noalias "unpacked"="0.4" %dst.45, %struct.parameters* noalias readonly "unpacked"="1" %src) unnamed_addr #2 {
entry:
...
copy:                                             ; preds = %entry
  ...
  call void @arraycpy_hls.p0a90a90f32([90 x [90 x float]]* %dst.01, [90 x [90 x float]]* %src.0, i64 90)
...
; Function Attrs: argmemonly noinline norecurse willreturn
define void @arraycpy_hls.p0a90a90f32([90 x [90 x float]]* %dst, [90 x [90 x float]]* readonly %src, i64 %num) local_unnamed_addr #3 {
entry:
  %0 = icmp eq [90 x [90 x float]]* %src, null
  %1 = icmp eq [90 x [90 x float]]* %dst, null
  %2 = or i1 %1, %0
...
for.loop:                                         ; preds = %for.loop, %for.loop.lr.ph
  ...
  %dst.addr = getelementptr [90 x [90 x float]], [90 x [90 x float]]* %dst, i64 0, i64 %for.loop.idx2
  %src.addr = getelementptr [90 x [90 x float]], [90 x [90 x float]]* %src, i64 0, i64 %for.loop.idx2
  call void @arraycpy_hls.p0a90f32([90 x float]* %dst.addr, [90 x float]* %src.addr, i64 90)
...
; Function Attrs: argmemonly noinline norecurse willreturn
define void @arraycpy_hls.p0a90f32([90 x float]* %dst, [90 x float]* readonly %src, i64 %num) local_unnamed_addr #3 {
entry:
  %0 = icmp eq [90 x float]* %src, null
  %1 = icmp eq [90 x float]* %dst, null
  %2 = or i1 %1, %0
  br i1 %2, label %ret, label %copy
...
for.loop:                                         ; preds = %for.loop, %for.loop.lr.ph
  %for.loop.idx2 = phi i64 [ 0, %for.loop.lr.ph ], [ %for.loop.idx.next, %for.loop ]
  %dst.addr = getelementptr [90 x float], [90 x float]* %dst, i64 0, i64 %for.loop.idx2
  %src.addr = getelementptr [90 x float], [90 x float]* %src, i64 0, i64 %for.loop.idx2
...
; Function Attrs: argmemonly noinline norecurse willreturn
define internal fastcc void @copy_out(%struct.parameters* noalias "unpacked"="0", [90 x [90 x float]]* noalias readonly "unpacked"="1.0" %.0, float* noalias nocapture readonly "unpacked"="1.1" %.1, i32* noalias nocapture readonly "unpacked"="1.2" %.2, float* noalias nocapture readonly "unpacked"="1.3" %.3, [9 x i224]* noalias readonly "unpacked"="1.4" %.4, %struct.results* noalias "unpacked"="2", i416* noalias readonly align 512 "unpacked"="3") unnamed_addr #4 {
entry:
  call fastcc void @onebyonecpy_hls.p0struct.parameters.209(%struct.parameters* %0, [90 x [90 x float]]* %.0, float* %.1, i32* %.2, float* %.3, [9 x i224]* %.4)
...
; Function Attrs: argmemonly noinline norecurse willreturn
define internal fastcc void @onebyonecpy_hls.p0struct.parameters.209(%struct.parameters* noalias "unpacked"="0" %dst, [90 x [90 x float]]* noalias readonly "unpacked"="1.0" %src.01, float* noalias nocapture readonly "unpacked"="1.1" %src.12, i32* noalias nocapture readonly "unpacked"="1.2" %src.23, float* noalias nocapture readonly "unpacked"="1.3" %src.34, [9 x i224]* noalias readonly "unpacked"="1.4" %src.45) unnamed_addr #2 {
entry:
...
copy:                                             ; preds = %entry
  ...
  call void @arraycpy_hls.p0a90a90f32([90 x [90 x float]]* %dst.0, [90 x [90 x float]]* %src.01, i64 90)
...
declare void @apatb_do_compute_hw([90 x [90 x float]]*, float*, i32*, float*, [9 x i224]*, i416*)

; Function Attrs: argmemonly noinline norecurse willreturn
define internal fastcc void @copy_back(%struct.parameters* noalias "unpacked"="0", [90 x [90 x float]]* noalias readonly "unpacked"="1.0" %.0, float* noalias nocapture readonly "unpacked"="1.1" %.1, i32* noalias nocapture readonly "unpacked"="1.2" %.2, float* noalias nocapture readonly "unpacked"="1.3" %.3, [9 x i224]* noalias readonly "unpacked"="1.4" %.4, %struct.results* noalias "unpacked"="2", i416* noalias readonly align 512 "unpacked"="3") unnamed_addr #4 {
entry:
  call fastcc void @onebyonecpy_hls.p0struct.parameters.209(%struct.parameters* %0, [90 x [90 x float]]* %.0, float* %.1, i32* %.2, float* %.3, [9 x i224]* %.4)
  call fastcc void @onebyonecpy_hls.p0struct.results(%struct.results* %1, i416* align 512 %2)
  ret void
...
define void @do_compute_hw_stub_wrapper([90 x [90 x float]]*, float*, i32*, float*, [9 x i224]*, i416*) #6 {
entry:
  %malloccall = tail call i8* @malloc(i64 32664)
  %6 = bitcast i8* %malloccall to %struct.parameters*
  %7 = alloca %struct.results
  call void @copy_out(%struct.parameters* %6, [90 x [90 x float]]* %0, float* %1, i32* %2, float* %3, [9 x i224]* %4, %struct.results* %7, i416* %5)
  call void @do_compute_hw_stub(%struct.parameters* %6, %struct.results* %7)
  call void @copy_in(%struct.parameters* %6, [90 x [90 x float]]* %0, float* %1, i32* %2, float* %3, [9 x i224]* %4, %struct.results* %7, i416* %5)
  ret void
}
